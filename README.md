...
The interface is translated into Arabic, French, German, Hebrew, Japanese,
...
If an older copy was built in the same directory, remove `build/` first. The
QML interface is embedded in the compiled KCM, so an old build directory can
retain outdated resources.
...
with `fakeroot/usr/`. This directory is only a packaging workspace; it is not a
